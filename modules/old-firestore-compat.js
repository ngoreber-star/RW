...
(function(global) {
    ...
    async function getDocs(queryRef) {
        const sb = await getSb();
        const parsed = parsePath(queryRef.collectionRef.paths);
        ...
        for (const c of queryRef.constraints) {
            if (c._type === 'where') {
                ...
                else if (c.op === 'in') q = q.in(c.field, Array.isArray(c.value) ? c.value : [c.value]);
        ...
        return {
            ...
            empty: rows.length === 0,
            size: rows.length,
            forEach(fn) { rows.forEach(r => fn({ id: r.id, data: () => r, exists() { return true; } })); }
        };
    }
    ...
    async function getDoc(docRef) {
        ...
        if (parsed.mode === 'doc' && parsed.table === 'public_order_codes') {
            ...
            const { data, error } = await sb.from('public_order_codes')
                ...
                .maybeSingle();
            if (error) throw error;
            return { id: parsed.docId, data: () => data || {}, exists() { return !!data; } };
        }
    ...
    function convertToSnake(obj) {
        ...
        for (const key of Object.keys(obj)) {
            if (mappings[key] && !(mappings[key] in obj)) {
                obj[mappings[key]] = obj[key];
                // Keep original key too for safety
            }
        }
    }
